...
- Optional Live Instructions reuses the finished editor-style chart and advances at complete row/round boundaries with Done and Back. It shows the current path, yarn, and compressed work; unresolved overlays disable Live, and confirmed progress resumes locally only for the same complete structured plan. — **your decision** (optional Live, shared visuals, local progress); **Agent's choice** (row/round prototype boundary and exact-plan reset)
