...
- Instructions is a peer workspace to Design. It opens on a read-only Overview, moves keyboard focus to the workspace heading, and pairs the finished chart with a structured row/round list; selecting a unit focuses its chart path without changing project data or progress. Landscape places chart and list side by side, while portrait stacks them. — **Agent's choice**
