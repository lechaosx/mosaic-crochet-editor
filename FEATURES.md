...
- Instructions is a peer workspace to Design. It opens on a read-only Overview, moves keyboard focus to the workspace heading, pauses Design shortcuts, and pairs the finished chart with a structured row/round list; selecting a unit focuses its chart path without changing project data or progress. Reactivating the current workspace preserves its view and session. Landscape places chart and list side by side, while portrait stacks them. — **Agent's choice**
...
- Optional Live Instructions reuses the finished editor-style chart and advances at complete row/round boundaries with Done and Back. It shows the current path, yarn, and compressed work; unresolved overlays disable Live, confirmed progress resumes locally only for the same complete structured plan, failed progress writes remain visibly warned, and keyboard completion focus moves from the removed final Done action to the completion heading. — **your decision** (optional Live, shared visuals, local progress); **Agent's choice** (row/round prototype boundary, exact-plan reset, scoped recovery warning, and completion focus)
...
- Compact More exposes standard menu semantics: opening focuses the first command, arrow and boundary keys navigate commands, and Escape restores the trigger. Commands return to ordinary button semantics in the wide document bar. — **Agent's choice**
