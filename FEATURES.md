...
- **Wand drag** sweeps through multiple regions: each new cell the cursor enters runs another wand select using the captured mode verbatim. Replace → each step replaces with the cell's region (drag previews what would commit on release, overshoot fixed by dragging back). Add → drag accumulates. Remove → drag peels off. One history entry covers the whole drag. — **your decision**
...
- **Move tool — Ctrl+drag = copy**: holding Ctrl at paintdown flips the lift's `wipe` flag, so the source cells are left intact and the float is a duplicate. Release commits the copy at the new position; the selection mask shifts to the copy too (the source is no longer in the selection). — **your decision**
- **Move tool — Shift+drag = move mask only**: holding Shift at paintdown flips the lift's `stamp` flag — the source isn't cut and no lifted pixels render or commit. The marquee outline tracks the cursor so the user can reposition just the selection. Release commits the shifted mask only. Caveat: Shift+Alt is also the OS keyboard-layout switcher, so for mask-only it's best to engage the Move tool via `M` / button click rather than via the Alt-swap. — **your decision**
