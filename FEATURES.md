...
### Pixel Painting
- Left click paints primary color, right click paints secondary color. — **Agent's choice**
- Painting is blocked on inner hole (transparent) pixels. — **Agent's choice**
- Strokes that change nothing are silently ignored — no history entry, no dirty mark. — **Agent's choice**

### Color Pickers
Click either color swatch to select it as active. Double-click (desktop) or long-press (touch / mouse, ~500 ms) opens the native color picker. Right-click on desktop still paints with the secondary colour. — **your decision (click-to-select, double-click + long-press); Agent's choice (unified pointer long-press)**
...
### Fill Tool
Flood-fill a contiguous region. Respects active symmetries. — **Agent's choice**

### Tool Selection
Pencil, Fill, Eraser buttons in the top bar. Keyboard shortcuts: **P** (pencil), **F** (fill), **E** (eraser). — **Agent's choice**
...
- Symmetry applies to both pencil, fill, and eraser. — **Agent's choice**
...
- Undo and Redo buttons in the top bar; disabled when operation is unavailable. — **your decision (buttons); Agent's choice (disabled state)**
...
Clicking **New** opens a popover anchored to the button (uses the native HTML `popover` attribute, so it light-dismisses on outside click and Esc). Every settings change immediately updates the canvas and commits (baseline reset, history reset, session saved). — **your decision (behaviour); Agent's choice (native popover)**
...
Clicking **Export** opens a modal. The pattern text appears line by line as it is generated, with a live progress counter in the header. Closing the modal cancels generation immediately. — **your decision (line-by-line, cancellation); Agent's choice (progress counter)**

- **Alternate direction** toggle below the modal header; re-generates immediately on change. — **your decision**
- **Copy to clipboard** and **Download .txt** buttons; disabled while generating. — **Agent's choice / your decision**
- Warning banner when invalid placements exist; export is not blocked. — **Agent's choice**
...
- **Save** — downloads the pattern as a `.mcw` file (JSON). On browsers with the File System Access API (Chrome/Edge), opens a native save dialog and resets the dirty baseline on success. On others (Firefox) the file downloads immediately with no confirmation possible. — **Agent's choice**
...
Tool, color, symmetry, and pixel state auto-save to `localStorage` and restore on refresh. — **Agent's choice**

---

## Touch Support

Single-finger drag draws (paint / fill / eraser, with active symmetry). Two-finger drag pans + pinch-zooms in one gesture. Long-press on a swatch opens the native colour picker. The whole UI uses pointer events, so touch, pen, and mouse share one code path. — **Agent's choice (gesture model, pointer-event unification)**

## Highlight Settings

Highlight overlay/invalid colour and opacity live in a popover triggered by the **⊙** button (in the same toolbar group as the rotation buttons). Tap the small swatches inside to open the colour picker; the opacity slider is below. — **Agent's choice (popover placement so the inputs no longer crowd the toolbar)**
...
Both breakpoints (single-row → two-row, and the start of the shrink) are derived at runtime from each group's actually-measured intrinsic width, so wrapping and shrinking happen exactly when content stops fitting — never sooner. — **your decision (layout shape, distribution, auto-shrink); Agent's choice (measure-driven thresholds)**
