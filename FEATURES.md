...
### Color Pickers
Click either color swatch to select it as active. Long-press (mouse or touch, ~500 ms) opens the native color picker. Right-click on desktop still paints with the secondary colour. — **your decision (click-to-select); Agent's choice (unified pointer long-press, dropping double-click)**
...
Highlight overlay/invalid colour and opacity live in a popover triggered by the **⊙** button in the toolbar. Long-press the small swatches inside to change colours. — **Agent's choice (popover placement so the inputs no longer crowd the toolbar)**
