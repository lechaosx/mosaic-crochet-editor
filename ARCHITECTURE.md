...
| `transform-evaluator.ts` | Mask-packed directional-grid evaluation, collision reporting, and source mapping | free functions |
...
- Packed-grid evaluation uses unbounded integer cell coordinates and returns its effective column/row steps, uniquely mapped output cells, and all multi-source collision coordinates. Zero spacing searches occupied source cells for the nearest non-overlapping placement after applying the configured cross-axis offset. — **your decision** (mask packing, directional counts, spacing, offsets, and collision rules); **Agent's choice** (pure evaluator contract)
