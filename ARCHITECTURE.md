...
- The fresh-session surface is transient UI over a concrete default `SessionState`, not a nullable document state. Choosing a Pattern geometry resets history around that concrete initial state and creates recovery immediately; canceled file selection leaves the surface untouched. — **Agent's choice**
