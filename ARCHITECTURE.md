...
- **wasm-pack `--target bundler`** — produces named ESM exports consumed directly by Vite; no init function needed, WASM loading handled automatically by `vite-plugin-wasm`. — **Agent's choice**
- **`base: "./"` in vite.config.ts** — relative asset paths so the site works on GitHub Pages subpaths. — **Agent's choice**
...
### `wasm`
Thin binding layer. Depends on `core`. Contains only `src/lib.rs` with wasm-bindgen entry points. `package.json` points `main` and `types` directly to `pkg/` output — justified since `pkg/` is an internal detail within the same package boundary. — **Agent's choice**
...
- **Discriminated union** (`RowState | RoundState`) for pattern state — TypeScript narrows correctly when accessing round-only fields. — **Agent's choice**
- **`el<T>(id)`** typed DOM helper — avoids scattered null assertions. — **Agent's choice**
...
Build order is mandatory: `core` → `wasm` → `web`. `bun install` works before any build since workspace membership is based on `package.json` presence, not `pkg/` existence. — **Agent's choice**; mandatory ordering identified by — **your correction**
...
GitHub Actions: push to `master` → install deps → `build:wasm` → `build:web` → deploy `web/dist/` to GitHub Pages via `actions/deploy-pages`. — **Agent's choice**; no custom packaging step — **your decision**
