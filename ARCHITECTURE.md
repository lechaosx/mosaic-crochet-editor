...
- `main.ts` retains one direction-neutral WASM instruction plan keyed by a 64-bit fingerprint of pattern geometry and visible cells. Cached WASM units own their compressed trees until a different pattern replaces the cache; presentation direction renders from those trees on demand. — **your decision** (pattern-only invalidation and immediate direction changes); **Agent's choice** (cache ownership and fingerprint boundary)
