...
- **Logic mutation:** Stryker with the Vitest runner (`bun run test:mutation`) mutates `logic/src/`. The September 2026 baseline is 1,167 mutants, 73.95% total mutation score, and 77.26% among covered mutants. Node executes Stryker because its instrumenter relies on CommonJS default-import unwrapping that Bun does not provide; `flake.nix` supplies Node for this script. — **joint** (runner); **Agent's choice** (recorded baseline)
- **E2E:** Playwright (pinned to 1.59.1) drives `vite preview` in desktop Chromium. Tests in `web/e2e/*.spec.ts` cover boot, tools, paint, selection/move/copy/cut/paste, symmetry, edit popover, and persistence. `render.ts` exposes the canvas matrix on `window.__test_matrix__` so cell-relative clicks don't need to inspect view state. Touch behavior is not covered by the current E2E project. — **Agent's choice**
