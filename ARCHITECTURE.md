...
- `core::export` derives public flat `WorkStep` records before rendering text. Each record carries its row/round identity, Yarn A/B slot, worked and parent coordinates, and stitch kind; the existing exporter groups and compresses those records without changing its text format. — **Agent's choice**
