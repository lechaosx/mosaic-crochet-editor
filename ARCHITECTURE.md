...
- `.mcw` file format is v3: packed pixel bits, Global Mirror axes, and bit-packed saved-repeat source masks and settings. New recipe mode, alternate-gap, centre, and turn fields remain optional on read so earlier v3 files default to Grid, inherit primary gaps, and use the source centre. Pure encode/decode lives in `@mosaic/logic/mcw`; the web layer only reads and writes text. Byte-to-base64 conversion is chunked so maximum-size masks do not exceed the JavaScript argument limit. Fixture tests preserve v1/v2 loading, and decoding validates version, shape, dimensions, transforms, and payload length before session replacement. — **your decision** (v1/v2/v3 compatibility and transform persistence); **Agent's choice** (codec boundary and validation)
