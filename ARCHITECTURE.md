...
- The fresh-session surface is transient UI over a concrete default `SessionState`, not a nullable document state. Its Pattern choices use the normal transaction, while initial preview and canceled creation skip document recovery until Apply, successful load, or example selection. — **Agent's choice**
