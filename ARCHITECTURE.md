...
- **`InstructionSession`** — `#[wasm_bindgen]` struct; JS owns it, calls `.next()` for typed row/round metadata, compressed text, yarn, and packed worked coordinates, then calls `.free()`. Avoids global session state and keeps the visible list and copied text on one generated sequence. — **Agent's choice**
...
- About uses a native modal `<dialog>` with a full-viewport light-dismiss surface, and More uses a light-dismiss popover. Pattern, Selection, Mirror & Repeat, and Settings are sections in one non-modal inspector host. — **Agent's choice** (dialog implementation); **your decision** (About behavior)
...
- Canvas context is interaction-owned rather than a persistent projection of `Store`: coordinates, selection state, paint previews, and coalesced rejected-action feedback update it only while relevant. — **your decision**
...
- State ownership is defined by meaning rather than by its current storage container:
  ...
  | Crochet progress | Confirmed crochet progress boundary | Browser-local recovery; progress-only Back changes this boundary and it remains excluded from `.mcw` and project Undo/Redo |
...
- About records the last dismissed changelog entry ID under `mosaic-about-changelog`; a missing or different value opens the dialog without depending on document recovery. Dated changelog IDs support continuous deployment without a release-version scheme. — **Agent's choice** (storage key and entry IDs); **your decision** (changelog-triggered display without semantic versions)
...
- `live-progress.ts` stores a versioned confirmed-unit count under `mosaic-live-progress`, keyed by a 64-bit fingerprint of every unit's label, yarn, text, and worked coordinates. The UI projects one additional current unit for the inclusive preview; a nonmatching or invalid record starts at the first unit, while a rejected write is reported in Crochet without changing document-recovery status. — **your decision** (current-line inclusive preview); **Agent's choice** (confirmed-count persistence)
- `ui.ts` measures visible mode panels and inspectors and publishes canvas-chrome inset properties. Navigation and the compact status cluster consume those insets, while the canvas remains full-workspace and unchanged. — **your decision** (panels over the canvas and chrome at visible edges); **Agent's choice** (ResizeObserver-backed CSS insets)
- Mode-owned chrome is marked declaratively in the DOM and filtered by the body workspace class. The persistent document bar is excluded from that filtering; its authoring callbacks close Crochet before mutating or opening Design context. — **Agent's choice**
...
- Hover preview and paint share the same read-only `evaluatePaintAt` path, including float write-back projection and lock-invalid filtering; preview computes a temporary highlight plan without mutating session or history. WASM exposes the core inward-cell rule so Overlay feedforward cannot disagree with placement geometry. — **Agent's choice**
...
- **E2E:** Playwright (pinned to 1.59.1) drives `vite preview`. Desktop Chromium covers the adaptive shell, tools, paint, selection/move/copy/cut/paste, symmetry/repeat, Pattern interactions, and persistence; a Pixel 7 Chromium project covers single-finger paint, two-finger pan/zoom cancellation, and modifier-free mask movement. `render.ts` exposes the canvas matrix on `window.__test_matrix__` so cell-relative input does not need to inspect view state. — **Agent's choice**
