...
| `selection.ts` | Float lift/cut/anchor/delete, symmetry replication, and `applySelectionMod` (rect / wand / select-all / deselect wrappers) | free functions |
...
- Pattern dimensions are validated in `@mosaic/logic/pattern` before Rust allocation: at most 16,777,216 canvas cells total and 1,048,576 cells on either axis. Edit, local-session restore, and `.mcw` load all use the same rule. The Rust orbit walker separately aborts an entire operation when a composed orbit would exceed 1,048,576 cells; an empty orbit is the internal abort sentinel, so tools return their original pixel buffer instead of partial symmetry output. — **joint** (ceilings); **Agent's choice** (validation boundary and abort sentinel)
...
- One-shot symmetry replication sends a canvas-sized source buffer and the active-axis wire format to Rust. Rust applies complete orbits atomically and returns an applied/unchanged/conflict/orbit-limit status; TS commits only copied canvas pixels, leaving the source float active. — **Agent's choice**
