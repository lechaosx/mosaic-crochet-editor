...
- Move-pixels is a **TS-orchestrated transient float**: `RendererState.float = { base, lifted, mask, dx, dy }` holds the lifted layer for the duration of a drag; the store stays untouched until release. Each render frame builds a stamped preview canvas (base + lifted at offset, dropping off-canvas / hole destinations to match commit semantics) and uses it as BOTH the visible pixel source AND the input to `build_highlight_plan_*` — so the ✕ / ! markers update live as the user drags rather than only at commit. Selection outline traces the shifted mask (clipped to canvas + non-hole). Commit at release builds the final pixels + selection in one snapshot; pointer-cancel just clears `rs.float`. Same struct shape will be reused for persistent floats in Phase 3 — `lifecycle = transient | persistent` is the only thing that differs. — **your decision**
- **Move tool gating**: the Move tool (`M`) is the only tool that triggers a lift; holding **Alt** at the keyboard layer swaps the active tool to Move (and restores on keyup / window blur). This collapses "temporary modifier override" into "the tool of record IS Move," so the gesture state machine doesn't need a per-tool modifier branch — `onPaintStart` / `onPaintAt` / `onPaintEnd` just check `activeTool === "move"`. — **your decision**
