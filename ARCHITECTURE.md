...
- State ownership is defined by meaning rather than by its current storage container:
  ...
  Recovery v6 contains only document and workspace state; `mosaic-preferences` owns app-global display settings. History v5 stores document, active selection, and the axis/repeat recipe as separate bounded fields; a future retained transform-source bitmap belongs to workspace recovery rather than every Undo snapshot. — **your decision** (ownership); **Agent's choice** (physical envelopes)
...
- Global Mirror axes: `SessionState.axes: Axis[]` — a list of mirror / rotation axes, each with a kind-specific position scalar. Fresh sessions start with `[]`; the user adds axes in the **Global mirrors** section of Mirror & Repeat (5 "+V/+H/+C/+D1/+D2" buttons) or with V/H/C/D/A keyboard shortcuts. Multiple axes of the same kind coexist (wallpaper symmetry). `symmetry.ts` compiles active axes to flat `(kind, a, b)` records via `axesToFlat`; `repeat.ts` appends the two repeat records through `transformsToFlat` before crossing into Rust. Rust builds the per-axis reflection closure first, then applies the Cartesian repeat offsets to that completed motif. The symmetry closure handles composition (V + H produces the 4-cell orbit; C isn't needed). Move-tool drag near an active guide starts an `axis-drag` gesture; `pickAxesAt` picks up to one axis per kind within the hit tolerance, so a click at an intersection grabs all crossing axes (parallel overlapping axes of the same kind get disambiguated by closeness). Each axis tracks the cursor along its own kind's projection. Release snaps to half-cell (V/H/C) or whole-cell (D1/D2). Dragging an axis to where it can no longer mirror two distinct in-canvas cells (`axisOffCanvas`) deletes it on release; the guide renders at reduced alpha while the cursor is in the dead zone. The inspector's per-axis row carries a toggle (active/inactive) and an × delete button. Project axes are grid-representable and useful for their saved pattern, so Pattern changes filter unusable axes before Save; `mcw.ts` validates that invariant and migrates v1/v2 files with `axes: []`. The project boundary never receives workspace, preference, progress, selection, or history state. The authoring dock groups `g-paint` (pencil / fill / eraser / invert), `g-overlay` (Place / Clear / Invert), `g-transform` (select / wand / move / Selection / Mirror & Repeat), and yarn controls. The WASM build needs `--enable-nontrapping-float-to-int` because the target generator's `f64 as i32` casts emit `i32.trunc_sat_f64_s`. — **Agent's choice** (axes refactor, inspector UX, wire encoding, project codec boundary); **your decision** (keep C as a kind, zero-axes default, add-to-list semantics, toolbar regroup, geometric delete trigger, snap-to-half/integer, symmetry-before-repeat order, persisted Global Mirror)
