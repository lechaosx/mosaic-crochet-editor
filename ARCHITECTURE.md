...
- Live WIP pixels are reconstructed in `core::export` from natural colours plus the completed `WorkStep` prefix. A completed `oc` changes its supporting cell only when its row/round enters that prefix; transparent future cells are ignored by highlight-plan generation. WASM returns the derived byte buffer to the non-persisted Instructions preview store. — **your decision** (temporally correct chart surface); **Agent's choice** (core replay and WASM buffer boundary)
- Instructions owns a separate, non-persisted preview `Store`, but reuses Design's `Viewport`, `RendererState`, rendering context, and DOM canvas. The chart-viewport element moves between mode layouts, preserving pan/zoom/rotation, while the preview store removes the live float and transform guides. `RendererState.focusPath` adds only the selected work path or blocker location. — **your decision** (one persistent viewport); **Agent's choice** (preview-store boundary)
- Overview and Live move that shared chart viewport between their layouts and force its pointer interaction into navigation. `live-progress.ts` stores a versioned completed-unit count under `mosaic-live-progress`, keyed by a 64-bit fingerprint of every unit's label, yarn, text, and worked coordinates; a nonmatching or invalid record starts at the first unit, while a rejected write is reported inside Live without changing document-recovery status. — **your decision** (mode-specific UI around the shared canvas); **Agent's choice** (navigation gating and progress storage)
