...
- `build:wasm` (wasm-pack, `--no-default-features` strips `console_error_panic_hook` from release) → `build:web` (Vite). `dev:rust` watches via `cargo-watch`. — **Agent's choice**
...
- GitHub Actions: single `ci.yml` — three parallel test jobs (`rust`, `web`, `e2e`), then `deploy` gated on all three + `master` push only. — **Agent's choice**; no custom packaging — **your decision**
