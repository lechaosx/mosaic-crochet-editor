...
- Repeat grid: `SessionState.repeat: RepeatGrid` stores inclusion, tile width/height, and copies per side on each axis. `SessionState.liveTransforms` independently gates whether `paintAt` passes the configured records to drawing tools; selection stamping always passes them. Repeat configuration remains session/history state through `.mcw` load and canvas resize, while the live flag is session-only workflow state. `renderRepeatGuides` draws dotted tile boundaries while the transform popover is open or repeat is included. — **your decision** (two execution paths and state lifetime); **Agent's choice** (module and renderer placement)
