...
Click **Pattern** to edit the pattern's dimensions and colours. Shape and size come first; below them, Yarn A/B use direct colour pickers and **⇄** exchanges the two colours as one undoable edit. Project danger and accent overrides sit beside the yarns because their contrast is judged against the current pattern. **Use default** removes one override, **Find contrasting colors** chooses a deterministic, visually distinct pair with at least 3:1 contrast against both yarns whenever the semantic palette permits it without adding an Undo step, and **Use for new patterns** saves the current pair as the app defaults.
...
Global Mirror's **Mirror while drawing** switch controls mirror axes. Each saved repeat has its own **Repeat while drawing** switch. The five drawing tools respect both active choices, and the eraser restores each affected pixel to *its own* natural colour rather than the click point's.
