...
- **Load** opens a file picker and restores pattern geometry, pixels, and colours. Symmetry and repeat transforms are session state and remain unchanged. Invalid files and files from a newer unsupported `.mcw` version are reported in the document bar without blocking the workspace or replacing the current pattern.
