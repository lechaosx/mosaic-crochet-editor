...
Authoring is tool-led rather than controlled by a global strategy switch. **Colour** contains Pencil, Fill, Eraser, and Invert; **Overlay** places the chart-required overlay operation; **Arrange** contains Select, Magic wand, Move, and Mirror & Repeat. The canvas context identifies the active group and tool. Wide layouts use a single-control-width Design tool rail; compact layouts retain the same ordered controls and active-context text in a bottom dock.
