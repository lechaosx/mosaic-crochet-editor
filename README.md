...
- **Design** and **Crochet** are peer modes in the document bar. They keep one canvas fixed in the workspace, including its pan, zoom, rotation, and navigation controls; switching changes only the overlay panel above it. The rest of the document bar stays available in both modes. Design shows the authoring panel. Crochet makes the canvas navigation-only and shows the complete crochet-order instruction list, with the current row or round highlighted. The canvas shows chart-derived work through the end of that current line exactly as it should look when the line is finished; future rows, rounds, and overlay contributions remain absent. **Done** advances one whole row or round and **Back** returns to the previous one. Progress resumes locally only when the complete regenerated instruction plan matches, is excluded from `.mcw` and project Undo, and reports a browser-storage failure inline. **Copy instructions** copies the complete compressed instruction dump without opening a separate text view. Toggle **Alternate direction** to regenerate the list. Each `oc` belongs to the worked row or round containing its visible ✕, while the covered supporting pixel remains an internal chart detail.
- Invalid overlay placements remain spatially linked blockers. The compressed copy remains an explicit draft: `?` marks unresolved work inside a generated row/round, and a coordinate-bearing unresolved line identifies every blocker, including work outside the generated chart path. Progress controls remain disabled until blockers are resolved.
...
The compact status cluster in the canvas's bottom-right corner shows the active tool and Yarn A/B, overlay totals, invalid placements, selection size, and whether configured transforms are live or paused. Pattern coordinates appear while the pointer is over the canvas. The selection or clipboard count opens its contextual action card. The cluster wraps when space is constrained.
