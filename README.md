...
- **Instructions** opens a peer workspace on **Overview** and moves keyboard focus to its heading. Design, Overview, and Live move the same chart canvas and view controls between their layouts, so pan, zoom, and rotation remain unchanged; the canvas is navigation-only outside Design. Activating the already-current Instructions command preserves its active view. Overview is the finished reference: its chart and crochet-order row/round list share focus, and selecting a work unit highlights its complete path without editing the pattern or recording progress. Its view tabs support Left/Right and Home/End navigation, and Design shortcuts remain paused until you return. The header, tabs, and generation options wrap instead of scrolling horizontally when enlarged text leaves insufficient width. Landscape layouts place chart and list side by side; portrait layouts stack them. **Live** changes the same canvas to the work-in-progress surface at the confirmed row/round boundary: completed work is solid, the current unit is a distinct pending path, future rows/rounds are absent, and a future overlay does not change its supporting cell early. **Done** advances one complete row/round and **Back** reconstructs the preceding surface; completing the final unit moves keyboard focus to the completion heading. The confirmed boundary resumes locally only when the complete regenerated instruction plan matches. Live progress is not saved in `.mcw` or project Undo, and Live remains unavailable while unresolved overlay positions make the plan unsafe. If browser storage rejects a progress update, Live warns you to keep the tab open. **Text** shows the exact compressed output used by Copy and Download, with a legend for `sc`, `ch`, and the chart-required `oc` operation. Copy and Download report completion beside their controls, and clipboard rejection is reported there too. Each `oc` belongs to the worked row or round containing its visible ✕, while the covered supporting pixel remains an internal chart detail. Toggle **Alternate direction** to regenerate all three views with the other work direction. **Back to Design** restores the editor without changing its selection or tools, then returns focus to **Instructions** or compact **More**.
